...
The declaration is **data, not Rust**, and that keeps assembly acyclic: B02
generates `crates/contracts/src/**` from the declarations, so no agent-owned
crate is ever a build input to the contract crate. Generated files are never
hand-edited — regeneration must reproduce them byte for byte. Collisions are a
hard failure at assembly naming both claimants: duplicate type name, error code,
exit code, config key, path id, IPC message tag or setting id. Assembly stops; it
does not resolve by last write.
...
- **Error convention** — Win32 signals failure five ways: a `BOOL`, an `HRESULT`,
  a null handle, `INVALID_HANDLE_VALUE`, and success with `GetLastError()` set.
  Two crates will read the same call differently.
...
A wrapper CCR states, and the wrapper documents: the exact Win32 function and its
header; who owns the returned handle and where `Drop` closes it; which failure
convention the call uses and its mapping into `contracts::Error`; the string
conversion in both directions; and the invariant each `unsafe` block relies on,
as a comment (REQ-FND-06).
...
error rather than leaving the service stopped. Each is a declared guarantee, not
a comment in B09's code.
