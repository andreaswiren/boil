...
| Load a skill on demand | Six skills hold the procedures: `build-orchestrate`, `contract-guard`, `version-guard`, `visual-qa-cdp`, `supply-chain-audit`, `release-build`. | `Skill`, or the `/<name>` form | **Observed 2026-09-22 on Muse Code 1.3.0: the skill did not load.** Whether it scans `.claude/skills` at all is `unconfirmed`, and so is `muse skills import --from claude` — both were recorded here as *confirmed* on an inference rather than on a run, which is the error this row now exists to correct. Resolve with `muse --help` and record what it prints. | No REQ is lost. A skill is a procedure, and a procedure can always be read as a file — which is why the entry point now leads with reading `prompts/00-master-orchestrator.md` rather than with a slash command. What is lost is cache economy, which shows up in REQ-COST-08's cache-read ratio. |
