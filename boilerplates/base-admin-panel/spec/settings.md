...
a certificate is. A new agent for a surface that already has an owner is the
...
Two scopes both looking accountable is not two settings: it is **one setting at
the wider scope with a narrower override** (§6). MFA is the canonical case —
*whether* it is required is global (REQ-AUT-05), *which* methods satisfy it is
tenant (REQ-AUT-04), *which factors you enrolled* is personal (REQ-AUT-06).
...
string, the panel renders on its write string** — narrower, never wider, and
deny-by-default makes that safe (REQ-RBA-02). And **a `global`-scope panel must
be gated by a `global.*` string** (REQ-RBA-06, `contracts/types/rbac.md` §3), or
a tenant admin holding a tenant-namespace string would be shown the global
scope, the disclosure REQ-SET-07 exists to prevent. `†` marks a grammar-valid
string (REQ-RBA-01) not yet in `rbac.md` §6 — an additive declaration by the
named owner (`rbac.md` §8, fast path); a permission that does not resolve in the
assembled registry denies, so the panel does not render.
...
**Store** is the descriptor's choice. `preferences` means A05's
`user_preferences` row, written through `PUT /api/v1/shell/preferences` with the
scope in the body — A07 never opens A05's table and A05 never interprets a
density value. `domain` means the owning domain's own table and operation; A05
renders and routes the submit, and writes nothing (REQ-CTR-04). Global-scope
*preference* defaults are not rows but the validated env values
(`SHELL_DEFAULT_TIMEZONE`, REQ-FND-07) — REQ-TIM-03's "system default".

## 3. Scope is legible before the save, not after (REQ-SET-06)

Four mechanisms, rendered by the shell from the descriptor so a panel author
cannot omit one:
...
1. **A scope chip beside every mutating control**, not only in the header:
   ...
   colour-only fails AA contrast and a colour-blind operator (REQ-UI-11).
2. **A distinct surface treatment per scope.** Personal is the default card;
   tenant adds a header band naming the tenant; global renders inset with a
   persistent left rail and the destructive accent from `theme-tokens`.
...
A heading that says *Global* is insufficient alone for three concrete reasons.
It is read once on entry while the click happens 800 px further down, out of
view. Panels are deep-linkable (REQ-SET-12), so a user can arrive mid-panel and
never see it. And on a phone the panel is a sheet whose header scrolls away
above the thumb arc holding the primary action (REQ-UI-07).
...
see returns `common.not_found` (404), not 403 — a 403 confirms the surface
exists (`contracts/types/errors.md` §4).

**An empty scope is worse than an absent one.** It advertises a capability the
actor does not hold, so it reads as a broken build rather than an access
boundary and earns a support ticket instead of an understanding. It discloses
the shape of the estate — that a global tier exists, and roughly what it
administers — to an actor holding nothing in it, the same leak 404-not-403
closes a layer down. And it invites a tenant admin to ask for the empty tab to
be "fixed": a privilege-escalation conversation started by a rendering bug.
...
`settings-registry`, which A05's shell reads, filters by permission, and mounts
by scope and order.
...
specifier. Same mechanism as `nav-registry` and help topic links: **registry,
never a shared list** (`contracts/ownership.md`). It keeps the wide wave
parallel, because two agents adding a panel in the same minute touch two
different files (REQ-CTR-04). A panel component imports `packages/contracts` and
its own package, nothing else (REQ-CTR-01).
...
the UI, declared by the owning domain at the scope that holds it.
...
panel shows the **effective value and its source**, phrased as the source and
not as a state — "Europe/Stockholm — your tenant's default", "Page size 50, from
your tenant's default" (`spec/time.md` §5, `spec/theming.md` §2,
`spec/datagrid.md` §7). Where `locked` is true the control is read-only and
carries the lock sentence naming the scope. It never posts and fails: the server
still refuses with `settings.locked_by_higher_scope` (422), but that path is for
the API caller, not the UI's way of telling a user no.
...
flipping to `tenant` on the next resolve. Any earlier tenant value was shadowed
by the lock, not deleted — which is why the algorithm short-circuits instead of
clearing the narrower row.
...
checkbox is not acceptable (REQ-AUT-05). The token is a locale-independent
uppercase ASCII literal declared by the field — `DISABLE MFA FOR ALL TENANTS` is
the shipped example — interpolated into a catalogue message, so it does not
breach REQ-I18N-02; a translated token would make the confirmation weaker in
Swedish than in English. A mismatch returns `settings.confirmation_mismatch`
(422).
...
effective: "immediate" | "next-login" }`, evaluated against the same predicate
the write will use and under the actor's own permissions, so it never names a
tenant the actor cannot see. It runs twice, for the inline count in §3 and
inside the confirmation; if the two disagree the save is refused with
...
confirmed change also records the token typed, the reason and `affectedCount` as
confirmed, so the record shows what the operator was told.

**The scope must be a field on the record, not an inference.** A diff alone
cannot answer "was MFA turned off for one tenant or for the estate" — the first
question after an incident, and the difference between a tenant admin exercising
their authority and an operator overriding it. Two identical diffs at two scopes
are two different events. REQ-SET-09 therefore needs
`AuditEvent.settingsScope: SettingsScope | null`, which
`contracts/events/audit-event.md` §1 does not publish and whose schema is
`.strict()`. A05 files it as an additive optional field on A13's envelope
(`contracts/README.md` §5); until A13 assembles it, no panel satisfies
REQ-SET-09. Both alternatives are worse: the scope in `target.id` turns a column
filter into a prefix match, and a correlated second event doubles the trail and
adds a pairing that breaks like an unpaired impersonation `enter`.

## 9. Search, deep links and help (REQ-SET-12)

- **Deep links.** Every panel is a route, `/(app)/settings/<scope>/<panel-id>`;
  a field anchors at `?f=<fieldId>`. Panel ids are unique by assembly, so the URL
  space needs no second registry. A link to a panel the recipient may not see
  resolves to 404, per §4.
- **Search.** A05 indexes every descriptor plus its fields' i18n keys in every
  shipped locale at build time and filters by permission at query time — a
  Swedish operator finds "kvarhållning", an English one "retention", same panel.
  The index holds labels and help text, never values. The same entries feed the
  command palette (REQ-UI-12).
- **Help.** `helpTopicId` is mandatory: a panel without one fails assembly, and
  A16 fails the documentation gate on an unresolvable id (REQ-DOC-03). Topics
  are localised and permission-aware (REQ-DOC-04).
...
- `pnpm test:contract` — `packages/contracts/tests/settings-registry.spec.ts`
  (REQ-CTR-10): descriptors parse; no duplicate panel id; every permission
...
- `pnpm test:permissions` — `tests/permission-denial/settings/**` (REQ-TST-05): a
  panel requested without its permission is absent from the payload; a scope with
  zero permitted panels is absent and its route returns 404; read-only permission
  on a write returns 403.
- `pnpm test:unit` — `tests/unit/settings/**`: the §6 algorithm over a table of
  (global, tenant, personal, lock) inputs, including a stored personal value
  under a global lock; source labelling; token matching, case and whitespace.
- `pnpm test:e2e` and `pnpm test:visual` — the §6 MFA example at all three
  scopes; a locked control read-only and never posting; a stale count returning
  `settings.blast_radius_changed`; a field deep link focusing its field; search
  by `sv` label; every scope at 390/834/1440 in light and dark with the scope
  chip in the viewport beside the submit control at 390 (REQ-TST-03).
- `pnpm test:audit` — `tests/audit-emission/settings/**`: every write emits
  `kind: "policy"` with a redacted diff and `settingsScope` equal to the panel's
  scope; a confirmed change records the token, reason and `affectedCount`.
- `pnpm lint:boundaries` plus `GET /api/v1/shell/_selftest` — no panel component
  imports outside `packages/contracts` and its own package (REQ-CTR-01); the
  registry is non-empty, every component and `helpTopicId` resolves, and every
