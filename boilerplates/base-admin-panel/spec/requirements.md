...
| REQ-FND-12 | MUST | A `.gitignore` is present **from the first commit**, not added once the tree is already dirty with build output. It excludes dependencies, build output, test and capture output, and — the line that matters — every secret-bearing file: `.env`, private keys, certificates. `REQ-SEC-03` keeps secrets encrypted at rest, which a committed `.env` makes moot, and a git history is not something you can un-leak by deleting the file later. |
| REQ-FND-13 | MUST | The `.gitignore` never excludes `build/`, `resources/`, `mockups/` or the self-hosted fonts. Each looks like build output and is not: `build/` is the conformity evidence `compliance/cra/obligations-matrix.md` cites at the strongest tier, `resources/` is the pinned baseline (`REQ-UI-16`), the mockups are the reference the approved design is checked against (`REQ-MOC-05`), and the fonts are an asset `REQ-SUP-07` forbids fetching at build time. A generic JavaScript template ignores `build/` by default, so this is checked rather than trusted. |
...
A build that reports progress it has not compiled is not reporting progress. These
requirements exist because "done" is the cheapest word an agent can write, and
nothing above this section forces it to be earned.

| ID | Status | Requirement |
|----|--------|-------------|
| REQ-VAL-01 | MUST | **One validation command, defined in Wave 0, used by everyone.** `pnpm validate` runs typecheck, lint, unit tests and build over the workspace; `pnpm validate --filter <pkg>` runs it over one package. Agents, gates, CI and the human run that command and no other. A gate that runs a different set from the one developers run is a gate that certifies trees which do not build. |
| REQ-VAL-02 | MUST | **Every hand-off is validated by the agent that wrote it, on the tree as it stands, immediately before the report.** The report carries a validation block: the exact command, the exit code, the duration, the commit sha, and the tail of the output. A claim of success with no command behind it is prose. |
| REQ-VAL-03 | MUST | **An unvalidated hand-off is not accepted.** The orchestrator rejects a report whose validation block is missing, non-zero, or taken against a different sha, and re-dispatches. The rejection is mechanical — it reads the block, it does not judge whether the work looks finished. |
| REQ-VAL-04 | MUST | **No agent states a build, typecheck, lint or test result it did not obtain by running the command in this session.** A result recalled from an earlier round is stale by construction: the tree changed, which is why there was another round. Reading the code and concluding it compiles is the specific failure this requirement names. |
| REQ-VAL-05 | MUST | **Every phase ends green over the whole tree, including the phases before the product exists.** The contract package typechecks at `G3`; the mockup workspace builds at `G1` (REQ-MOC-07); the scaffold builds at `G2`. A phase that produces no runnable artefact still produces a compiling one, and a phase boundary is the cheapest place to find out that it does not. |
| REQ-VAL-06 | MUST | **Warnings are errors at every gate**: `tsc --noEmit` with zero errors under `strict` and `noUncheckedIndexedAccess` (REQ-FND-03), ESLint at `--max-warnings=0`, `cargo clippy -- -D warnings` for the Rust agent (REQ-OBS-01). A warning tolerated at `G4` is a warning nobody reads at `G8`, because by then there are four hundred. |
| REQ-VAL-07 | MUST | **Suppression is a waiver, not a tactic.** Every `@ts-expect-error`, `eslint-disable`, `#[allow(...)]`, `// nosec` and `--no-verify` carries the REQ ID it trades against and the condition for its removal. The count is reported at every gate and **a count that rose since the previous gate is a finding**, because the ordinary way a red tree turns green is not a fix. |
| REQ-VAL-08 | MUST | **The validated tree, the captured tree and the reviewed tree are one tree.** Every validation record, every screenshot sidecar (REQ-CAP-03) and every gate verdict names a commit sha, and a gate whose evidence spans more than one sha does not pass. Six verdicts spread across three commits certify a tree no reviewer saw, and whatever landed between them is unreviewed. |
| REQ-VAL-09 | MUST | **Integration is validated against the real dependency.** The database in an integration run is a real PostgreSQL over `sslmode=verify-full` (REQ-SEC-03); the browser is a real Chromium over CDP (REQ-TST-02); the mail path is the real relay container. A mock proves the code calls something, which is never the claim being made. |
| REQ-VAL-10 | MUST | **The live instance is rebuilt from the validated tree and never left broken.** A failing validation rolls the instance back to the last green build and the status page says so, naming the failing command and the agent. A URL serving a broken build teaches the human to stop looking at the URL, which costs more than the failure did (REQ-LIV-01, REQ-LIV-06). |
| REQ-VAL-11 | MUST | **A red shared tree stops dispatch.** When validation fails in the contract package, the workspace config or another shared file, no new task is dispatched into that wave until it is green — fifteen concurrent agents building on a broken base produce fifteen hand-offs that all have to be redone, and the first one to notice is always the gate. |
| REQ-VAL-12 | MUST | **The validation history is kept, not summarised.** `build/validation/<wave>/<agent>.json` per hand-off and `build/validation/<gate>.json` per gate, each with command, exit code, counts, duration, sha and output tail. The human can then see that the tree went green at each step, which is the difference between progress and a claim of progress. |
| REQ-VAL-13 | MUST | **Generated artefacts are regenerated and diffed, never trusted.** The typed API client, the contract fixtures (REQ-CTR-06), the traceability matrix and the compliance set are regenerated during validation and a non-empty `git diff` fails it. A generated file edited by hand is a fork of its source that nothing will ever reconcile. |
| REQ-VAL-14 | MUST | **Validation covers the deployment artefact, not only the source.** The Docker image builds, the compose stack comes up, migrations apply from empty and `/api/health/ready` returns healthy (REQ-FND-04, REQ-FND-10) — at `G5` and at every gate after it. A workspace that typechecks and an image that starts are two different claims. |

## CAP — Continuous visual capture

Screenshots in this build are a **feed**, not a deliverable produced twice. The
human is watching a build that runs for hours; a capture set that arrives at `G1`
and again at `G5` leaves everything between them unobserved.

| ID | Status | Requirement |
|----|--------|-------------|
| REQ-CAP-01 | MUST | **Capture is continuous.** Every wave, every gate and every landed UI change produces a capture set — not only at a gate, and never only on failure. A surface nobody has looked at since the wave it was written in is a surface nobody has looked at. (Supersedes the timing half of REQ-TST-03.) |
| REQ-CAP-02 | MUST | **A stable path convention**, so that two captures of the same surface sort next to each other and a regression is visible by scrolling: `build/screenshots/<wave>/<surface>__<viewport>__<theme>__<sha>.png`. Surfaces are named from the route, not invented per run. |
| REQ-CAP-03 | MUST | **Every image has a sidecar** `.json` beside it: URL, viewport, theme, commit sha, timestamp, the REQ IDs the surface serves, the interaction that preceded it (REQ-CAP-09), and the console and network errors observed during capture. A screenshot that cannot be tied to a tree is a picture, not evidence. |
| REQ-CAP-04 | MUST | **The feed is delivered both ways, every time**: the images are presented in the chat reply (REQ-TST-04) **and** written to `build/screenshots/`. Not one or the other — the reply is immediate and scrolls away, the folder persists and nobody watches it. Dropping either loses immediacy or history, and both were asked for. |
| REQ-CAP-05 | MUST | **The live instance serves the feed** at `/_build/screenshots`: newest first, grouped by surface, each image next to its sidecar and its previous capture of the same surface. The human scrolls the build's visual history at the URL they already have (REQ-LIV-01), without a checkout and without asking an agent for it. |
| REQ-CAP-06 | MUST | **Captures come from the live instance** (REQ-LIV-03) via Playwright over CDP against a production build (REQ-TST-02). Never a second ephemeral server, never a `file://` path, never `next dev` — a dev-mode render carries the overlay and hot-reload client, so it is a picture of the toolchain. |
| REQ-CAP-07 | MUST | **A capture carries its console.** Page errors, unhandled rejections and failed requests are recorded during every capture, and **a surface captured with a console error is reported as failing**, not presented as a screenshot that happens to look right. The screenshot is the thing most likely to hide a broken page that renders. |
| REQ-CAP-08 | MUST | **Every set is three viewports × two themes** — 390 / 834 / 1440 px, light and dark (REQ-TST-03) — for every surface in the set. Desktop-light alone is the single configuration least likely to be broken and therefore the least informative one to capture. |
| REQ-CAP-09 | MUST | **Capture after interaction, not only on load.** Each surface's set includes its loaded state and at least its populated, working state: a grid with a filter and a sort applied and page two reached (REQ-GRD-02 … REQ-GRD-09), a form at its validation-error state, a dialog open, the sidebar collapsed. An empty initial render proves the route resolves and nothing about the feature. |
| REQ-CAP-10 | MUST | **A gate does not pass on a stale capture.** Every surface the gate covers has a capture at the sha under review (REQ-VAL-08). Capture itself never blocks a wave — it runs alongside — but it blocks the gate, because that is the moment the evidence is being relied on. |
...
| REQ-TST-03 | MUST | Screenshots are captured at multiple points during a run — not just at failure — across mobile, tablet and desktop, in light and dark. The viewport and theme matrix stands; the timing is **superseded by REQ-CAP-01**, which makes capture continuous rather than per-run. |
| REQ-TST-04 | MUST | Screenshots are presented in the chat response to the user for every build that touches the UI. **Extended by REQ-CAP-04**: presenting them in the reply is necessary and not sufficient — the durable folder and the served feed are the other two halves. |
...
| REQ-TST-09 | MUST | **A feature is not done until a test fails without it.** The owning agent records both runs — the red one before the change and the green one after — in its validation block (REQ-VAL-02). A test written afterwards against code that already passes it asserts the code's present behaviour, which is a different claim from the requirement it cites. |
| REQ-TST-10 | MUST | **Tests land in the same task as the behaviour, never in a later cleanup.** Each domain agent writes the tests for its own behaviour as it writes it; `A23` owns the shared fixtures, the contract interface tests and the cross-cutting suites. There is no phase in this build where testing catches up, because the phase that would be cut under pressure is the one that catches up. |
| REQ-TST-11 | MUST | **Every `MUST` maps to a test, or to a named reason it cannot be tested.** `build/validation/req-coverage.md` is generated from the register and the suites, and at `G8` a `MUST` with neither a test nor a recorded reason fails the gate (REQ-REL-07). An untested `MUST` is an unverified claim wearing the word MUST. |
| REQ-TST-12 | MUST | **No test is skipped, quarantined, `.only`-scoped or marked pending to reach green.** The counts of skipped and focused tests are reported at every gate; non-zero is a gate finding naming each test. A suite of 400 tests where 40 are skipped reports as green and verifies 360. |
| REQ-TST-13 | MUST | **The suite runs every wave, not only at `G4` and `G5`**: the targeted suite on every hand-off (REQ-VAL-02), the full suite at every wave boundary. A regression found three waves after it landed costs the three waves built on top of it. |
| REQ-TST-14 | MUST | **A flaky test is a defect with an owner.** A test that fails and then passes unchanged is investigated and fixed, not re-run until green; if it is quarantined it is quarantined once, dated, with the owning agent named, and it blocks `G8`. "Re-run it" is how a real intermittent failure becomes a production incident. |
| REQ-TST-15 | MUST | **Test output is kept as evidence**, not paraphrased: `build/validation/<wave>/<agent>/test-output.txt` with the runner's own passed/failed/skipped counts. A sentence claiming the suite passed is not the suite passing, and the two are indistinguishable in a report. |
| REQ-TST-16 | MUST | **Security and isolation suites run at every gate from `G4` onward**, not once at `G5`: tenant isolation, permission denial, MFA enforcement and audit emission (REQ-TST-05). These are the four that fail silently — nothing about the UI changes when RLS stops applying. |
