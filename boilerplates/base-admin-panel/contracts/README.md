...
| `agent-report` | [`types/agent-report.md`](types/agent-report.md) — every agent's structured hand-off, including token usage where `null` means unreported and never zero (REQ-COST-01) | A02 |
| `editor` | [`types/editor.md`](types/editor.md) — the language and formatter registry behind every text-editing surface (REQ-MON-01, REQ-MON-03) | A05 |
| `mobile-primitives` | [`types/mobile-primitives.md`](types/mobile-primitives.md) — bottom nav, sheets, reach zones, and the `TouchBudget` that extends A05's surface budget (REQ-MOB-02) | A27 |
| `impersonation` | [`types/impersonation.md`](types/impersonation.md) — the impersonation session, the effective-permission rule, and the refusal taxonomy (REQ-IMP-02, REQ-IMP-07) | A04 |
